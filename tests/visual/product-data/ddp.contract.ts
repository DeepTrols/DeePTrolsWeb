...
export function registerDdpVisualContracts() {
  it('keeps the DDP product page aligned with its requirement document', () => {
    ...
    const unifiedFlow = readComponent('components/demo/ddp-unified-development/DdpUnifiedDevelopmentFlow.client.vue')
    ...
    expect(unified).toContain('DdpUnifiedDevelopmentFlow')
    expect(unified).toContain('<ClientOnly>')
    expect(unified).toContain('relative z-[1] mx-auto h-full w-[min(1704px,100%)] overflow-hidden')
    expect(unified).toContain('h-[560px] w-[1704px]')
    expect(unified).toContain('-translate-x-1/2 -translate-y-1/2')
    expect(unified).toContain(':viewport-y="16"')
    expect(unified).toContain('label="统一数据开发流程图"')
    expect(unified).toContain('fallback-text="统一数据开发流程图加载中"')
    expect(unified).not.toContain('占位符')
    expect(unified).not.toContain('<style')
    expect(unifiedFlow).toContain('viewportY: 26')
    expect(unifiedFlow).toContain("type: 'ddpUnifiedDevelopmentCurve'")
    expect(unifiedFlow).not.toContain('<style')
