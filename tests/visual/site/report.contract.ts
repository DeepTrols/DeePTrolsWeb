...
export function registerReportVisualContracts() {
  it('renders the report service page with the required resources layout', () => {
    ...
    expect(section).toContain('load-more-link')
    ...
    expect(data).toContain("{ key: 'data-infrastructure', label: '数据设施' }")
    expect(data).toContain("{ key: 'knowledge-engineering', label: '知识工程' }")
    expect(data).toContain("{ key: 'smart-manufacturing', label: '智能制造' }")
    expect(data).toContain("{ key: 'smart-water', label: '智慧水利' }")
    expect(data).toContain("{ key: 'smart-education', label: '智慧教育' }")
    expect(data).toContain("{ key: 'fde', label: 'FDE' }")
    expect(data).toContain("{ key: 'compute-power', label: '算电协同' }")
