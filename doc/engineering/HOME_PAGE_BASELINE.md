...
12. HOME About 标题不要主动插入换行，保持完整标题一行排版；窄屏仅允许浏览器按容器宽度自然换行。About section 不套用全局 `.section` 的大底部 padding，外层使用 Tailwind v4 `pb-5` 收住 `partner-rows` 后的空白；下方 `Resources` section 使用 Tailwind v4 `pt-16 lg:pt-24` 承接 section 间距。
