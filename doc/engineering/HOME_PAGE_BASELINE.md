...
4. 常规页面 Hero 后续模块遵循「Section 间距统一规则」（TASK-014.14，见 `COMMON_SECTION_COMPONENTS.md`）：section 根统一加 `flow-root pb-32 lg:pb-44`（底部 128px，lg 以上 176px，默认不写 pt），内部 `.container` 不携带垂直 margin；不要在页面 scoped CSS 中重复声明 section 垂直 padding，旧的 `dt-section` / `--dt-space-section(-lg)` 节奏已删除。HOME 专属例外：`HomeInsights` 根节点额外携带 `pt-32`，`HomeAbout` 底部收窄为 `pb-10`；`HomeDeliverables` 板块已从首页卸载（组件与数据保留，待重新要求时再挂载），hero 下首个 section 为 `HomeProductSystem`，其顶部间距经 `ProductSystemSection` 透传 `SectionShell` 的 `paddedTop` opt-in（根追加 `pt-32`）承接。
5. Header 高度固定为 `62px`，使用 `fixed` 全视口顶部，层级为 `z-index: 1000`；首页与暗色 Hero 页面顶部默认透明并使用白色 Logo/白色文字；所有产品页顶部默认透明并使用黑色 Logo/黑色文字；滚动、Mega 展开或移动菜单展开时切换为白色半透明背景与黑色 Logo/黑色文字。`is-in-hero` 只用于识别 HOME Hero 区域，不再触发深色半透明 Header 背景。
6. Header 是全视口宽度，内部 padding 按 DeepCtrls Header 节奏：桌面 `padding-left: 43px`、`padding-right: 61px`；普通页面内容仍使用 `.container`。
7. Header logo 使用运行时品牌图：首页与暗色 Hero 顶部透明状态为 `/images/brand/deeptrols-logo-white.png`，产品页顶部透明状态、滚动、Mega 展开或移动菜单展开时为 `/images/brand/deeptrols-logo-black.png`；Header logo 高度按参考站为 `35px`。Footer logo 当前宽度为 `240px`。
...
2. Mega Menu 只在 hover 对应按钮及面板时保持显示，离开按钮和面板后必须收回。Mega 参考 DeepCtrls：左侧 `.mega-title`，右侧文字入口；核心产品使用 `.mega-cols` 双栏联动（默认 `display: none` 隐藏三级 `.mega-col--links` 产品栏、只显示二级 `.mega-col--categories` 品类栏，`.mega-cols` hover 或 focus-within 时产品栏展开，品类栏 hover/focus 切换产品栏内容；二级条目的 `>` 箭头默认 `opacity: 0`，仅该条目 hover/focus-visible 时显示；三级条目不带 `>` 箭头；面板打开按当前路由激活对应品类，无匹配时激活第一个品类；品类栏不含 `核心产品` 入口），解决方案使用 `.mega-solutions` 横向 `4 × 2` 网格，`hot` 标签随标题 inline-flex 排版、紧贴标题文字右上角。条目标题 `24px / 35px`，描述 `14px / 16px`，hover/focus/active 仅将标题切换为蓝色，不使用图标卡片。
