...
- 常规区块间距遵循「Section 间距统一规则」：section 根统一 `flow-root pb-32 lg:pb-44`，内部 container 不携带垂直 margin（见 `COMMON_SECTION_COMPONENTS.md`）；解决方案页不启用 `SectionShell` 的 `paddedTop` opt-in。
