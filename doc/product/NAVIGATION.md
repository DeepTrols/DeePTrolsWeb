...
- `行业案例`（`/cases`）与 `TokenHub`（`/services/token-hub`）为顶级直链，不展示 Mega Menu。
...
- 导航 active 归属：智慧教育 `/services/smart-education`、FDE `/services/enterprise-ai-delivery` 与 `/solutions/fde` 均归入「解决方案」主导航高亮；Mega 内对应链接通过 `activePaths` 识别别名路由。

## 行业案例
```text
直接跳转 `/cases`（行业案例页），不展示 Mega Menu。
白皮书 & 报告路由 `/resources/reports` 保留，可通过页脚「白皮书&报告」链接直达。
```
