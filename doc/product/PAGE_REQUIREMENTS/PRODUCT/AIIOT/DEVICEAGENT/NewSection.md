...
全链路可观测排版微调补充（2026-08-24）：Trace 顶部三个指标 label 使用 `text-xs leading-5`，指标值使用 `text-sm leading-5`；7 个 Trace Span 进程外层间距使用 `gap-2`，单个进程内部间距使用 `gap-5`；Observability 标题使用 `text-sm font-semibold`，说明文案使用 `mt-2 text-xs leading-5`，确保说明文字可读且不压缩到旧的紧凑行高。
