...
流水线卡片状态右置与 IconBox 图标补充（TASK-008.7）：按反馈调整三张流水线卡片——1）状态移至卡片右侧；2）标题（事件校验 / 事件去重 / 上下文构建）统一为 `text-sm font-semibold text-highlighted`（白色，不随 pending 变灰）；3）描述统一为 `mt-1 text-xs leading-5 text-muted`（附 title 悬停全文）；4）图标改用公共 `IconBox` 组件（`size=40` / `icon-size=20`，圆边框 + 背景），pending 使用 `tone="soft"`、active/done 使用 `tone="primary"`，active 阶段保持 `animate-pulse`。响应式：xl+ 为行内布局（IconBox ｜ 标题+描述 ｜ 右侧状态）并显示 ArrowRight 流线；xl 以下（lg 面板列约 560px，无法容纳行内布局）回退为纵向堆叠（IconBox + 标题 + 状态，隐藏描述与流线），面板高度预算不变。
