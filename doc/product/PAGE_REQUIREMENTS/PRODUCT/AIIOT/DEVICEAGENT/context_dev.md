...
区域间距 mt-3 → mt-5 补充（TASK-008.15）：按反馈「间距mt-3修改为mt-5」调整——两面板共 5 处区域间距统一加大：事件面板事件条卡与流水线行、上下文面板进度条行、来源网格、序列卡均由 `mt-3` 改为 `mt-5`。`RuntimePanelShell` 外壳 `h-[520px]` → `h-[530px]`（`xl:h-[480px]` 不变）：事件面板 <xl 堆叠增至 340px，移动/lg 内容盒 347px 容纳（7px 余量），sm 367px、xl+ 317px ≥ 300px；上下文面板来源网格 <xl flex-1 铺满（两行各约 82–84px）、xl+ flex-none 自然高度 80px。harness-check.mjs 与 visual.spec.ts 同步更新 shell 高度与 mt-5 字面量断言。CDP 实测 5 断点两面板 scrollH ≤ clientH 无溢出。时间轴、Active 轮转、进度条联动、卡片三行内容与 footer 文案不变。
