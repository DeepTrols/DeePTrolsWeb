...
修订说明二（TASK-008.1 视觉微调）：hero 视觉三层卡片宽度增加（顶部 320→384、中间 420→504、底部 360→432，等比 ×1.2）；板块1 数字条条目样式放大——数值 `text-2xl font-semibold text-highlighted lg:text-3xl`、标签 `mt-1 text-xs text-muted lg:text-sm`、卡片 `rounded-xl border border-default bg-dt-bg-soft/50 px-4 py-4 text-center backdrop-blur-sm`（需求中的 `bg-muted/50` 因 `bg-muted` 为 `@utility` 不支持透明度修饰符，等价替换为 `@theme` 颜色 `bg-dt-bg-soft/50`，渲染值相同）。

修订说明三（TASK-008.1 hero 视觉内容扩充）：①顶部卡片「链接生态」在数曜前新增 DeepSeek、千问、智谱、Kimi 四个 AI 生态 logo（新建 `deepseek-logo.svg` / `qwen-logo.svg` / `zhipu-logo.svg` / `kimi-logo.svg`，经 `data/tanyao.ts` `?url` 导入，统一 `IconBox :size="40" tone="soft"`），顶部卡片宽度 384→460；②中间卡片边缘节点在策略执行后新增 Node-RED（Workflow / red）与 Neuron（BrainCircuit / emerald），网格 `grid-cols-3` → `grid-cols-5`，`EdgeNode` 新增 `pulse` / `pulseLine` 字段，`v-else` 脉冲对改为按节点取色（策略执行 blue、Node-RED red、Neuron emerald）；中间卡片宽度 `w-[504px] xl:w-[560px]`（lg 画布仅 520px，xl 起放大），节点内边距 `p-3 xl:p-4`、边缘计算 broker 条 `w-3 xl:w-3.5` 保证五节点在 lg 画布内不溢出；③底部设备卡新增 PLC（CircuitBoard）与摄像头（Cctv），网格 `grid-cols-3` → `grid-cols-5`，宽度 432→460。harness 与 visual.spec 同步断言新文案/宽度/响应式类，tanyao-content.spec 补充 4 个新 logo 资源存在性检查。

修订说明四（TASK-008.1 视觉右缘对齐博曜基准）：`page-hero__visual` 右侧与博曜保持一致——取消 `flush-visual-end`（不再以 `lg:-mr-4 + justify-self-end` 越过 container 内容边界），`TanyaoHeroVisual` 根节点去掉 `max-w-xl` / `2xl:max-w-2xl` 上限（改 `hidden w-full lg:block`），画布由 `mx-auto` 改为 `ml-auto`（`pointer-events-none relative ml-auto h-[600px] w-full max-w-[600px]`）。至此探曜与博曜（`ml-auto max-w-[680px]`）几何一致：lg 下画布填满视觉列，xl/2xl 下画布右缘始终贴齐 container 内容右缘，不出现右侧空隙。harness 与 visual.spec 同步更新（TanyaoHero 断言不含 `flush-visual-end`，画布断言改 `ml-auto`，新增根节点无 max 上限断言）。
