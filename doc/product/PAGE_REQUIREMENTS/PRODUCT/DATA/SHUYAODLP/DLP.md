...
右侧：
使用 AI 辅助建标动画，固定展示窗口三色标题栏，并呈现“业务需求 → AI 语义理解 → 标签定义 / 对象模型 / 加工规则生成 → 规则校验 → 模型发布”的完整过程。动画自动循环，终态停留 1.2 秒；当前步骤使用 primary，完成步骤使用 emerald，待执行步骤使用 muted，所有步骤始终保留且不得改变面板高度。
