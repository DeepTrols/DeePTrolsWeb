...
右侧视觉替换为三组可循环的核心能力动画，保持现有左右交替排版与文字区域不变：

1. 多源数据统一接入：展示 MySQL、ERP、Excel、Kafka 四类数据源，依次完成连接检测、Schema 识别、字段映射和数据接入；字段类型冲突使用 amber，最终生成统一数据源目录。
2. 数据标准与质量治理：展示客户、订单、设备三类数据表的空值、重复值和格式错误治理过程，质量评分从 72 提升到 96。
3. 数据资产沉淀与服务：展示资产目录、血缘关系、权限审批 Stepper 和 API 服务发布结果。

统一状态语义为：当前 primary、完成 emerald、异常 amber、待执行 muted。所有步骤始终存在，完成后图标切换为 Check；动画自动循环，终态停留 1.2 秒，不得溢出容器或改变卡片高度。
