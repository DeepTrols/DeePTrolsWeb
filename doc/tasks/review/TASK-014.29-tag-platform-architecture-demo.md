...
## 完成说明
已完成独立数据标签平台架构 Demo，并复用于数曜·数据标签平台解决方案区。保留参考结构的 640px 高度、径向背景、卡片尺寸、圆角、阴影和动态连线，仅替换需求指定的内容及图标。后续按验收意见移除方格背景与顶部命名空间浮层，并将内部流程画布统一为数据治理产品架构使用的 1704px 基准；流程主体宽度调整为 1320px，左右主连接间距均为 100px，并重新校准整体居中位置。
