...
- 修正四卡片必须仍然保留 Stepper 语义：卡片与 ArrowRight 连接始终存在，仅根据时间线切换状态；当前步骤 `primary`，已完成步骤 `emerald`，未执行步骤 `muted`。
- 修正已完成步骤的 IconBox 图标：完成后由原步骤图标切换为 `CircleCheck`（✓），当前和未执行状态仍显示各自功能图标。
