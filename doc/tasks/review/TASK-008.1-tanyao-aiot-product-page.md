...
2. 新建 `components/product/tanyao/TanyaoHero.vue`：公共 `PageHero`（badge=探曜·AI物联感知平台 / RadioTower / title-line=连接、感知、智能 / title-gradient=AIoT智能物联底座 / visual-label=TANYAOIOT_HORE_WEBM / visual-size=large），`#after-actions` 插入 `HeroStatsStrip(:columns="4")`，`#visual` 插入 `TanyaoHeroVisual`。
...
4. 扩展 `components/common/HeroStatsStrip.vue`：新增 `columns?: 3 | 4` prop（默认 3，向后兼容），`4` 时使用 `grid-cols-2 sm:grid-cols-4`。
...
| `components/common/HeroStatsStrip.vue` | 新增 `columns` prop 支持 1×4 |
