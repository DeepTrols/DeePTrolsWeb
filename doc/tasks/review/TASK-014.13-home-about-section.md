...
8. 按 Review 反馈将 section 标题不主动换行沉淀为公共规则：`SectionHeader` 与 `.dt-section-title` 桌面端默认不换行，少数直写 section 标题补充 Tailwind v4 `whitespace-nowrap`。
...
|----------------|----|
| Branch         | main |
| Commit Message | fix(TASK-014.13): prevent section titles wrapping |
| Commit Hash    | 本轮提交见 Git 历史 |

## 完成说明
已完成首页 About section 新增与 HOME 基线同步；Review 反馈中标题不主动换行、所有 section 标题不主动换行、About/Resources 间距和 partner-rows 底部留白已优化。
