...
defineProps<{
  data: DeviceNodeData | EdgeNodeData | ClusterNodeData | IntegrationNodeData
}>()
...
<template>
  <div
    ...
    :class="['relative flex h-20 w-[190px] flex-col items-center gap-2.5 rounded-2xl border bg-white px-4 py-3 shadow-lg', deviceToneClasses[data.tone]]"
  >
    <h3 class="whitespace-nowrap text-xs font-bold leading-tight text-dt-text-highlighted">{{ data.label }}</h3>
    <div class="flex gap-3">
      <component
        :is="icon"
        v-for="(icon, index) in data.icons"
        :key="index"
        :class="['size-7', deviceIconClasses[data.tone]]"
        aria-hidden="true"
      />
    </div>
  ...
  <div v-else-if="data.kind === 'edge'" class="relative w-[170px]">
    ...
    <div class="relative flex flex-col rounded-2xl border border-dashed border-[#189bfe]/50 bg-white px-4 py-3 text-left shadow-lg">
  ...
  <div v-else class="relative flex w-[320px] flex-col gap-3 rounded-2xl border border-accented bg-white p-5 shadow-lg">
