...
import { datacenterHero } from '~/data/solutions/datacenter'
</script>

<template>
  <section class="relative h-[586px] overflow-hidden bg-[#1f49e5]" aria-labelledby="datacenter-hero-title">
    <img
      class="absolute inset-0 h-full w-full object-cover object-center"
      :src="datacenterHero.image"
      :alt="datacenterHero.imageAlt"
    />
    <div
    ...
    <div class="container relative h-full">
      ...
      >
        <h1
          id="datacenter-hero-title"
          class="m-0 bg-[linear-gradient(104.09deg,#1e44e0_-15.53%,#ffffff_36.62%)] bg-clip-text text-[clamp(28px,2.5vw,48px)] font-medium leading-[clamp(38px,3.75vw,72px)] tracking-[0] whitespace-nowrap text-transparent [-webkit-text-fill-color:transparent] max-md:text-[32px] max-md:leading-[1.32] max-md:whitespace-normal"
        >
          {{ datacenterHero.title }}
        </h1>
      </div>
      <div
        class="absolute left-0 top-[clamp(132px,16.67vw,320px)] w-[min(760px,100%)] max-w-full max-md:top-[290px]"
      >
