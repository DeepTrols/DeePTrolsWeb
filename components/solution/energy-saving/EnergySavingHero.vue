...
import { energySavingHero } from '~/data/solutions/energy-saving'
</script>

<template>
  <section class="relative h-[586px] overflow-hidden bg-[#1f49e5]" aria-labelledby="energy-saving-hero-title">
    <img
      class="absolute inset-0 h-full w-full object-cover object-center"
      :src="energySavingHero.image"
      :alt="energySavingHero.imageAlt"
    />
    <div
    ...
    <div class="container relative h-full">
      ...
      >
        <h1
          id="energy-saving-hero-title"
          class="m-0 bg-[linear-gradient(104.09deg,#1e44e0_-15.53%,#ffffff_36.62%)] bg-clip-text text-[clamp(28px,2.5vw,48px)] font-medium leading-[clamp(38px,3.75vw,72px)] tracking-[0] whitespace-nowrap text-transparent [-webkit-text-fill-color:transparent] max-md:text-[32px] max-md:leading-[1.32]"
        >
          {{ energySavingHero.title }}
        </h1>
      </div>
      <div
