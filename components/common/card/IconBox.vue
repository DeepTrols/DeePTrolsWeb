 ...
 * - tone：primary / muted / white / gradient / soft / success（soft 为 bg-soft 半透明底 + line 内描边）
...
withDefaults(
  defineProps<{
    ...
    tone?: 'primary' | 'muted' | 'white' | 'gradient' | 'soft' | 'success'
    bordered?: boolean
    filled?: boolean
  }>(),
  {
...
.icon-box--tone-success {
  --dt-icon-box-bg: rgba(52, 211, 153, 0.1);
  --dt-icon-box-color: rgb(52, 211, 153);
  --dt-icon-box-shadow: inset 0 0 0 1px rgba(52, 211, 153, 0.2);
}
